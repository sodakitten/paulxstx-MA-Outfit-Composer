...
namespace Paulxstx.LastOpOutfitPlugin.Editor
{
    ...
    {
        private const string AddMenuPath = "GameObject/最终操作换装/添加通用混搭换装生成器";
        private const string AddAndBuildMenuPath = "GameObject/最终操作换装/添加并生成通用混搭菜单";
        private const string ToolsAddMenuPath = "Tools/最终操作换装/给选中角色添加通用混搭生成器";
        ...
        private static void AddAndBuildFromGameObjectMenu(MenuCommand command)
        {
            ...
            catch (Exception e)
            {
                ...
                EditorUtility.DisplayDialog("最终操作换装 - 生成失败", e.Message, "确定");
            }
        }
        ...
        private static LastOpOutfitComponent AddOrGetComponent(GameObject go)
        {
            if (go == null)
            {
                EditorUtility.DisplayDialog("最终操作换装", "请先选中角色根节点。", "确定");
                return null;
            }
            ...
            if (component == null)
            {
                ...
                Undo.RecordObject(component, "添加最终操作换装生成器");
            }
    ...
    public class LastOpOutfitComponentEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            if (targets != null && targets.Length > 1)
            {
                EditorGUILayout.HelpBox(
                    "检测到同时选中了多个“最终操作换装生成器”。\n\n" +
                    "为了避免多个角色配置互相写乱，本插件不支持多对象同时编辑。请只选中一个生成器对象进行配置和生成。\n\n" +
                    "这个提示只是不让你多选编辑，不影响单个角色正常使用。",
                    MessageType.Warning);
                return;
            }
            ...
            EditorGUILayout.HelpBox(
                ...
                "本版默认不会自动给套装项目切换生成部件开关，避免生成变慢、更新后配置被补乱；已经手动配置好的部件开关会继续保留。",
                MessageType.Info);
            ...
            // 自动修正改到 OnValidate、生成按钮和显式按钮里执行。

            绘制分组标题("角色根节点");
            绘制属性("avatarRoot", "角色根节点");

            绘制分组标题("生成资源");
            绘制属性("outputFolder", "生成资源目录");
            ...
            var initializedNewTopLevelItems = 初始化新增项(config, superCountBefore, mixCountBefore);
            ...
            if ((changedByInspector || initializedNewTopLevelItems) && 修复复制新增项编号(config))
            {
                EditorUtility.SetDirty(config);
                serializedObject.Update();
            }
            ...
            绘制分组标题("安全工具");
            EditorGUILayout.HelpBox(
                "建议在大量修改或升级插件前先导出 JSON 备份。\n" +
                "冲突检测只做检查和提示，不会自动改你的配置。",
                MessageType.None);

            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("导出当前配置备份 JSON", GUILayout.Height(30)))
            {
                serializedObject.ApplyModifiedProperties();
                LastOpOutfitBackupUtility.ExportConfig(config);
            }

            if (GUILayout.Button("导入配置备份 JSON", GUILayout.Height(30)))
            {
            ...
            if (GUILayout.Button("冲突检测 / 配置检查", GUILayout.Height(30)))
            {
                serializedObject.ApplyModifiedProperties();
                LastOpOutfitConflictChecker.ShowReport(config);
            }

            if (GUILayout.Button("修复复制新增项编号（D 后继续 E/F/G）", GUILayout.Height(28)))
            {
                Undo.RecordObject(config, "修复复制新增项编号");
                if (修复复制新增项编号(config))
                {
                    config.RefreshDerivedData();
                    EditorUtility.SetDirty(config);
                    serializedObject.Update();
                    EditorUtility.DisplayDialog("最终操作换装", "已修复明显由复制新增造成的重复名称/重复参数值。", "确定");
                }
                else
                {
                    EditorUtility.DisplayDialog("最终操作换装", "没有发现需要修复的复制新增项。", "确定");
                }
            }
            ...
            if (GUILayout.Button("根据混搭项目第一层级补充子级部件开关", GUILayout.Height(28)))
            {
                ...
                EditorUtility.SetDirty(config);
                serializedObject.Update();
                EditorUtility.DisplayDialog("最终操作换装", "已补充生成混搭子级关闭部件开关：" + added + " 个。\n\n本按钮不会补充套装项目切换的部件开关。", "确定");
            }

            if (config != null && config.allowManualGenerateSuperSwitchCloseOptions)
            {
                if (GUILayout.Button("手动根据套装项目切换第一层级补充部件开关", GUILayout.Height(28)))
                {
                    var added = LastOpOutfitGenerator.AutoGenerateCloseOptionsFromFirstLevelChildren(config, true, true);
                    LastOpOutfitGenerator.EnsureObjectSlots(config);
                    EditorUtility.SetDirty(config);
                    serializedObject.Update();
                    EditorUtility.DisplayDialog("最终操作换装", "已手动补充套装项目切换/混搭项目的关闭部件开关：" + added + " 个。", "确定");
                }
            }

            EditorGUILayout.Space(12);
            if (GUILayout.Button("生成套装项目混搭换装系统", GUILayout.Height(38)))
            {
                try
                {
                    config.RefreshDerivedData();
        ...
        private void 绘制套装项目切换列表(LastOpOutfitComponent config)
        {
            ...
            groups.isExpanded = EditorGUILayout.Foldout(groups.isExpanded, "套装项目切换列表（数量：" + groups.arraySize + "）", true);
            if (!groups.isExpanded)
            {
                if (GUILayout.Button("+ 添加套装项目切换", GUILayout.Height(26)))
        ...
        private void 绘制混搭项目列表(LastOpOutfitComponent config)
        {
            ...
            groups.isExpanded = EditorGUILayout.Foldout(groups.isExpanded, "混搭项目列表（数量：" + groups.arraySize + "）", true);
            if (!groups.isExpanded)
            {
                if (GUILayout.Button("+ 添加混搭项目", GUILayout.Height(26)))
                    AddMixGroup(groups);
                return;
            }
        ...
        private void 绘制自定义一键全关按钮列表(LastOpOutfitComponent config)
        {
            ...
            buttons.isExpanded = EditorGUILayout.Foldout(buttons.isExpanded, "自定义一键全关按钮（数量：" + buttons.arraySize + "）", true);
            if (!buttons.isExpanded)
            {
                if (GUILayout.Button("+ 添加一键全关按钮", GUILayout.Height(26)))
        ...
        private void DrawSuperSwitchItemsList(SerializedProperty items, LastOpOutfitComponent config, string parentName)
        {
            ...
            if (!items.isExpanded)
            {
                if (GUILayout.Button("+ 添加套装选项", GUILayout.Height(24)))
        ...
        private void DrawDefaultMixValuesCompact(SerializedProperty list, LastOpOutfitComponent config, string title)
        {
            ...
            if (GUILayout.Button("+ 添加默认混搭设置", GUILayout.Height(22)))
            {
        ...
        private void DrawGameObjectList(SerializedProperty list, string title, string help)
        {
            ...
            if (GUILayout.Button("+ 添加物体引用框", GUILayout.Height(22)))
            {
                var idx = list.arraySize;
                list.InsertArrayElementAtIndex(idx);
        ...
        private void DrawCloseOptionsCompact(SerializedProperty list, string title)
        {
            ...
            if (!list.isExpanded)
            {
                if (GUILayout.Button("+ 添加关闭部件开关", GUILayout.Height(22)))
                    AddCloseOption(list);
                return;
            }
        ...
        private bool 初始化新增项(LastOpOutfitComponent config, int oldSuperCount, int oldMixCount)
        {
            ...
            {
                Undo.RecordObject(config, "初始化新增套装项目切换");
                for (var i = oldSuperCount; i < config.superSwitchGroups.Count; i++)
                {
                    初始化套装项目切换(config, config.superSwitchGroups[i], i);
                }
                changed = true;
            }
            ...
            {
                Undo.RecordObject(config, "初始化新增混搭项目");
                for (var i = oldMixCount; i < config.mixGroups.Count; i++)
                {
                    初始化混搭项目(config.mixGroups[i], i);
                }
                changed = true;
            }
        ...
        private static void 初始化套装项目切换(LastOpOutfitComponent config, SuperSwitchGroupOption item, int index)
        {
            if (item == null) return;
        ...
        private static void 初始化混搭项目(MixGroupOption item, int index)
        {
            if (item == null) return;
        ...
        private static bool 修复复制新增项编号(LastOpOutfitComponent config)
        {
            if (config == null) return false;
            ...
            foreach (var group in config.superSwitchGroups ?? new List<SuperSwitchGroupOption>())
            {
                ...
                changed |= 修复套装选项复制编号(config, group.items, parentName);
            }
            ...
            foreach (var group in config.mixGroups ?? new List<MixGroupOption>())
            {
                ...
                changed |= 修复混搭选项复制编号(group.items, parentName);
            }

            return changed;
        }

        private static bool 修复套装选项复制编号(LastOpOutfitComponent config, List<SuperSwitchItemOption> items, string parentName)
        {
            ...
            for (var i = 0; i < items.Count; i++)
            {
                ...
                changed |= 修复关闭项复制编号(item.closeOptions);
            }

            return changed;
        }

        private static bool 修复混搭选项复制编号(List<PartOption> items, string parentName)
        {
            if (items == null) return false;
        ...
        private static bool 修复关闭项复制编号(List<ClothesCloseOption> closeOptions)
        {
    ...
    public static class LastOpOutfitBackupUtility
    {
        ...
        public static void ExportConfig(LastOpOutfitComponent config)
        {
            if (config == null)
            {
                EditorUtility.DisplayDialog("最终操作换装", "没有找到配置组件。", "确定");
                return;
            }

            config.RefreshDerivedData();

            var fileName = "最终操作换装配置备份";
            if (config.avatarRoot != null && !string.IsNullOrWhiteSpace(config.avatarRoot.name))
                fileName += "_" + SafeFileName(config.avatarRoot.name);

            var path = EditorUtility.SaveFilePanel("导出最终操作换装配置备份", Application.dataPath, fileName + ".json", "json");
            ...
            EditorUtility.DisplayDialog(
                "最终操作换装",
                "配置备份已导出：\n" + path + "\n\n" +
                "备份里保存的是相对角色根节点的物体路径。下次导入时，请先把“角色根节点”设置成同一个模型或相同层级结构的模型。",
                "确定");
        }
        ...
        public static bool ImportConfig(LastOpOutfitComponent config)
        {
            ...
            var path = EditorUtility.OpenFilePanel("导入最终操作换装配置备份", Application.dataPath, "json");
            ...
            {
                Debug.LogException(e);
                EditorUtility.DisplayDialog("最终操作换装 - 导入失败", "JSON 读取失败：\n" + e.Message, "确定");
                return false;
            }

            if (data == null || data.formatVersion <= 0)
            {
                EditorUtility.DisplayDialog("最终操作换装 - 导入失败", "这个 JSON 看起来不是本插件导出的配置备份。", "确定");
                return false;
            }

            Undo.RecordObject(config, "导入最终操作换装配置备份");
            Apply(config, data);
            config.RefreshDerivedData();
            EditorUtility.SetDirty(config);
            ...
            EditorUtility.DisplayDialog(
                "最终操作换装",
                "配置备份已导入。\n\n" +
                "如果有物体没有恢复引用，通常是因为模型层级或物体名字和备份时不一致。建议马上点一次“冲突检测 / 配置检查”。",
                "确定");
    ...
    public static class LastOpOutfitConflictChecker
    {
        public static void ShowReport(LastOpOutfitComponent config)
        {
            ...
            EditorUtility.DisplayDialog("最终操作换装 - 冲突检测", dialogText, "确定");
        }
            ...
            List<string> warnings)
        {
            foreach (var group in config.superSwitchGroups ?? new List<SuperSwitchGroupOption>())
            {
                ...
                if (string.IsNullOrWhiteSpace(group.parameterName))
                    errors.Add("套装项目切换「" + group.displayName + "」参数名为空。");
                else
            ...
            foreach (var group in config.mixGroups ?? new List<MixGroupOption>())
            {
                ...
                if (string.IsNullOrWhiteSpace(group.parameterName))
                    errors.Add("混搭项目「" + group.displayName + "」参数名为空。");
                else
                {
        ...
        private static void CollectObjectIssues(LastOpOutfitComponent config, List<string> warnings, List<string> infos)
        {
            ...
            foreach (var pair in owners.Where(kv => kv.Value.Count > 1))
            {
                ...
                {
                    warnings.Add("同一个物体被多个关闭部件开关控制，可能互相覆盖：\n" +
                                 GetObjectPath(config.avatarRoot, pair.Key) + "\n  - " + string.Join("\n  - ", pair.Value));
                }
                else
                {
                    infos.Add("同一个物体出现在多个主体选项中。若这是共用部件，可以忽略：\n" +
                              GetObjectPath(config.avatarRoot, pair.Key) + "\n  - " + string.Join("\n  - ", pair.Value));
                }
            }
        }
    ...
    public static class LastOpOutfitGenerator
    {
        ...
        public static void Build(LastOpOutfitComponent config, bool showDialog)
        {
            try
            {
                EditorUtility.DisplayProgressBar("最终操作换装", "准备生成，请稍等...", 0.02f);

                if (config != null && config.autoGenerateCloseOptionsFromFirstLevelChildren)
                {
                    EditorUtility.DisplayProgressBar("最终操作换装", "正在自动补充混搭子级部件开关...", 0.08f);
                    AutoGenerateCloseOptionsFromFirstLevelChildren(config, false);
                }

                EditorUtility.DisplayProgressBar("最终操作换装", "正在检查配置...", 0.14f);
                EnsureObjectSlots(config);
                ValidateConfig(config);
                EnsureFolder(GetActualOutputFolder(config));

                EditorUtility.DisplayProgressBar("最终操作换装", "正在检测 Modular Avatar...", 0.20f);
                ...
                EditorUtility.DisplayProgressBar("最终操作换装", "正在创建管理对象和参数...", 0.28f);
                ...
                foreach (var super in superGroups)
                {
                    ...
                    EditorUtility.DisplayProgressBar("最终操作换装", "正在生成套装项目切换菜单：" + super.displayName, progress);
                ...
                {
                    var progress = 0.32f + 0.30f * (doneMenuGroups / (float)totalMenuGroups);
                    EditorUtility.DisplayProgressBar("最终操作换装", "正在生成混搭菜单：" + group.displayName, progress);
                ...
                EditorUtility.DisplayProgressBar("最终操作换装", "正在生成 FX 动画控制器...", 0.70f);
                var fx = CreateGenericMixFx(config);

                EditorUtility.DisplayProgressBar("最终操作换装", "正在接入 MA Merge Animator...", 0.86f);
                ConfigureMergeAnimator(GetOrAddComponent(manager, maMergeAnimatorType), fx);

                EditorUtility.DisplayProgressBar("最终操作换装", "正在保存资源...", 0.94f);
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();

                EditorUtility.DisplayProgressBar("最终操作换装", "生成完成", 1.0f);
                ...
                if (showDialog)
                {
                    EditorUtility.DisplayDialog(
                        ...
                        "本版默认不会自动生成套装项目切换的部件开关；已手动配置的部件开关会继续使用。\n" +
        ...
        private static void ValidateConfig(LastOpOutfitComponent config)
        {
            ...
            if (config.avatarRoot == null) throw new Exception("角色根节点为空。");

            var parameterNames = new List<string>();

            foreach (var super in GetSuperGroups(config))
            {
                if (string.IsNullOrWhiteSpace(super.displayName)) throw new Exception("套装项目切换名称不能为空。");
                if (string.IsNullOrWhiteSpace(super.parameterName)) throw new Exception("套装项目切换「" + super.displayName + "」参数名不能为空。");
                if (string.IsNullOrWhiteSpace(GetLastAppliedParameterName(super))) throw new Exception("套装项目切换「" + super.displayName + "」内部已应用参数名不能为空。");
            ...
            {
                if (string.IsNullOrWhiteSpace(group.displayName)) throw new Exception("混搭项目名称不能为空。");
                if (string.IsNullOrWhiteSpace(group.parameterName)) throw new Exception("混搭项目「" + group.displayName + "」参数名不能为空。");
        ...
        private static void ValidatePartValues(string label, List<int> values)
        {
            if (values.Any(v => v <= 0 || v >= 255))
                throw new Exception(label + "参数值只能使用 1-254。0 是“不穿/关闭”，255 是“保持当前状态”。");
        ...
        public static int AutoGenerateCloseOptionsFromFirstLevelChildren(LastOpOutfitComponent config, bool forceSupplement, bool includeSuperSwitchGroups = false)
        {
            ...
            Undo.RecordObject(config, "自动补充混搭子级关闭部件开关");
            var added = 0;
        ...
        private static GameObject PrepareManagerObject(LastOpOutfitComponent config)
        {
            ...
            Undo.RegisterCreatedObjectUndo(go, "创建最终操作换装管理器");
        ...
        private static void ConfigureMAParameters(Component component, LastOpOutfitComponent config)
        {
            ...
            {
                Debug.LogWarning("[最终操作换装] 未能自动写入 MA Parameters，请手动确认参数。");
            }
        ...
        private static void ConfigureObjectToggle(Component component, List<TargetState> targets, GameObject avatarRoot)
        {
            ...
            if (array == null)
            {
                Debug.LogWarning("[最终操作换装] 未能识别 MA Object Toggle 目标列表字段：" + component.name);
                return;
            }
        ...
        private static GameObject CreateChild(Transform parent, string name)
        {
            ...
            Undo.RegisterCreatedObjectUndo(go, "创建换装菜单插槽");
        ...
        private static void EnsureFolder(string folder)
        {
            ...
            if (parts.Length == 0 || parts[0] != "Assets")
                throw new Exception("生成资源目录必须以 Assets 开头。");
